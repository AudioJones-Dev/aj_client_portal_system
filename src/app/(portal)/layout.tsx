...
import { getClient, getCurrentUser, getPendingApprovals } from "@/lib/data";
...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  const [user, client, pending] = await Promise.all([
    getCurrentUser(),
    getClient(),
    getPendingApprovals(),
  ]);
  ...
  return (
    <div className="flex min-h-screen bg-bg">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <Topbar user={user} client={client} pendingCount={pending.length} />
